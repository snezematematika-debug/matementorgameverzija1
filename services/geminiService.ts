...
export const generateBatchProblems = async (topic: string, grade: string, count: number = 30): Promise<QuizQuestion[]> => {
  try {
    const prompt = `
      Генерирај ${count} математички задачи за вежбање за ${grade} одделение на тема: "${topic}".
      Задачите треба да бидат со различна тежина (Лесно, Средно, Тешко).
      Секоја задача треба да има 4 понудени одговори, точен одговор и кратко објаснување.

      ${MATH_INSTRUCTION}

      Врати го одговорот во JSON формат како низа од објекти:
      {
        "problems": [
          { "question": "...", "options": ["...", "...", "...", "..."], "correctAnswerIndex": 0, "explanation": "...", "difficulty": "Лесно" }
        ]
      }
    `;
...
export const generateFullLessonPackage = async (topic: string, grade: string): Promise<LessonPackage> => {
  try {
    // 1. Generate Lesson, Quiz, Scenario, Board Plan, Worksheet, Project
    const packagePrompt = `
      Креирај комплетен образовен пакет за ${grade} одделение на тема: "${topic}".
      Пакетот треба да ги содржи следните компоненти:
      1. Лекција (Наслов, цели, содржина со Markdown и SVG).
      2. Квиз (5 прашања со опции и објаснувања + Рубрика за оценување).
      3. Сценарио за час (Стандарди, воведна, главна и завршна активност).
      4. План за табла (Краток и прегледен со LaTeX формули).
      5. Работен лист (Стандарден формат).
      6. Проект (STEAM или реална примена).
      7. Вертикална поврзаност (Како оваа лекција се надоврзува на претходните знаења и што следи).
      ...
      Врати го одговорот во JSON формат со следнава структура:
      {
        "lesson": { "title": "...", "objectives": ["...", "...", "..."], "content": "..." },
        "quiz": { 
          "questions": [
            { "question": "...", "options": ["...", "...", "...", "..."], "correctAnswerIndex": 0, "explanation": "...", "difficulty": "Средно" }
          ],
          "rubric": "..." 
        },
        "scenario": { "topic": "...", "standards": "...", "content": "...", "introActivity": "...", "mainActivity": "...", "finalActivity": "...", "resources": "...", "assessment": "..." },
        "boardPlan": "...",
        "worksheet": "...",
        "project": "...",
        "connectivity": "...",
    ...
    const packageResponse = await callGeminiWithRetry({
      model: 'gemini-3-flash-preview',
      contents: packagePrompt,
      config: {
        systemInstruction: SYSTEM_PERSONA,
        responseMimeType: "application/json",
      }
    ...
    incrementDailyQuota().catch(e => console.error("Quota increment failed:", e));
    trackGeneration({
      contentType: 'full_lesson_package',
      topic,
      grade,
      model: 'gemini-3-flash-preview'
    }).catch(e => console.error("Generation tracking failed:", e));
