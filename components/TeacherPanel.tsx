...
import { GradeLevel, AppMode } from '../types';
...
interface TeacherPanelProps {
  grade: GradeLevel;
  setMateMachineProblem?: (problem: string) => void;
  setMode?: (mode: AppMode) => void;
}

type Tool = 'pen' | 'line' | 'eraser';
type PanelTab = 'NOTEBOOK' | 'MATE_MACHINE';

const TeacherPanel: React.FC<TeacherPanelProps> = ({ grade, setMateMachineProblem, setMode }) => {
  const [activeTab, setActiveTab] = useState<PanelTab>('NOTEBOOK');
  ...
  return (
    <div className="space-y-4">
      <div className="border-b pb-4 print:hidden">
        ...
        <p className="text-slate-500 mt-1">Алатки за подготовка и изведување на наставата.</p>
      </div>

      {/* Tabs */}
      <div className="flex gap-2 p-1 bg-slate-100 rounded-xl w-fit print:hidden">
        <button
          onClick={() => setActiveTab('NOTEBOOK')}
          className={`px-6 py-2 rounded-lg font-bold text-sm transition-all ${activeTab === 'NOTEBOOK' ? 'bg-white text-teal-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
        >
          📓 Интерактивна тетратка
        </button>
        <button
          onClick={() => setActiveTab('MATE_MACHINE')}
          className={`px-6 py-2 rounded-lg font-bold text-sm transition-all ${activeTab === 'MATE_MACHINE' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
        >
          ⚙️ Мате-машина (Задачи)
        </button>
      </div>
        ...
        <div className="space-y-6 animate-fade-in">
          <div className="bg-indigo-50 p-8 rounded-[2rem] border border-indigo-100">
            <h3 className="text-xl font-black text-indigo-900 mb-4 flex items-center gap-2">
              <span className="bg-indigo-600 p-2 rounded-lg text-white">⚙️</span> Генерирање задачи за Мате-машина
            </h3>
            <p className="text-indigo-700 mb-6">Изберете лекција од Алгебра за да генерирате задача која учениците ќе можат да ја решаваат чекор-по-чекор.</p>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                    <label className="block text-sm font-bold text-indigo-900/60 mb-1">Лекција</label>
                    <select 
                        value={selectedTopic}
                        onChange={(e) => setSelectedTopic(e.target.value)}
                        className="w-full p-3 border-2 border-indigo-100 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none bg-white font-medium"
                    >
                        {availableTopics.map(topic => (
                        <option key={topic.id} value={topic.name}>{topic.name}</option>
                        ))}
            ...
            <button
                onClick={fetchTask}
                disabled={loading || !selectedTopic}
                className="bg-indigo-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-indigo-700 shadow-lg transition-all transform active:scale-95 flex items-center gap-2"
            >
                {loading ? 'Се генерира...' : '⚙️ Генерирај алгебарска задача'}
            </button>
          </div>
          ...
          {task && !loading && (
            <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-100 space-y-6">
              <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Генерирана задача:</h4>
              ...
              <div className="flex gap-4">
                <button
                  onClick={() => {
                    if (setMateMachineProblem && setMode) {
                      setMateMachineProblem(task.problem);
                ...
                >
                  ЛАНСИРАЈ ВО МАТЕ-МАШИНА <ArrowRight className="w-6 h-6" />
                </button>
              </div>
            </div>
          )}
